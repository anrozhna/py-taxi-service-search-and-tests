...
class DriverListTest(TestCase):
    ...
    def test_lists_all_drivers(self):
        response = self.client.get(f"{DRIVER_LIST_URL}?page=2")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context["driver_list"]), 3)

    def test_driver_get_context_data(self):
        response = self.client.get(f"{DRIVER_LIST_URL}?username=test")
        self.assertEqual(response.status_code, 200)
    ...
    def test_drivers_get_queryset(self):
        response = self.client.get(
            f"{DRIVER_LIST_URL}?username=test_username_1"
        )
    ...
    def test_drivers_search_pagination(self):
        response = self.client.get(f"{DRIVER_LIST_URL}?username=test&page=2")
        self.assertEqual(response.status_code, 200)
        self.assertTrue("is_paginated" in response.context)
        self.assertTrue(response.context["is_paginated"] is True)
        self.assertEqual(len(response.context["driver_list"]), 3)
    ...
    def test_drivers_search_no_page(self):
        response = self.client.get(f"{DRIVER_LIST_URL}?username=nonexistent")
        self.assertEqual(response.status_code, 200)
